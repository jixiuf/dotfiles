...
if [[ $working_directory =~ $regex ]]; then
  ...
  #
   $term $other_args $termexec $SHELL -i -c "ssh -t $user@$host \"cd $path && exec "'\$SHELL'"\" &&exec $SHELL"
  ...
  path=${BASH_REMATCH[5]}
  # 执行alacritty时移除--working-directory参数，并添加-e ssh ${USER}@${HOST} cd ${Path}&& exec $SHELL
  $term $other_args $termexec $SHELL -i -c "ssh -t $user@$host -p $port \"cd $path && exec "'\$SHELL'"\" &&exec $SHELL"
else
